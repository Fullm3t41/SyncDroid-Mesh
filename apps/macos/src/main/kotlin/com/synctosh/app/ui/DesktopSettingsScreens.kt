...
fun BackgroundOperationScreen(
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        LazyColumn(
            ...
            item {
                Text(
                    "Keep SyncTosh available for scheduled synchronization without leaving its main window open.",
                    style = MaterialTheme.typography.bodyLarge,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
            }
            item {
                ...
                SettingsCard {
                    SettingsActionRow(
                        ...
                        detail = when {
                            !launchAtLogin -> "SyncTosh starts only when you open it"
                    ...
                    SettingsActionRow(
                        ...
                        detail = if (noBackgroundService) "Closing the window fully quits SyncTosh" else "Closing keeps SyncTosh in the menu bar",
            ...
            item {
                SettingsInformationCard(
                    if (noBackgroundService) {
                        "SyncTosh will stop discovery and synchronization when its window closes. Open the app again whenever you want to resume."
                    } else {
                        "Closing the SyncTosh window keeps synchronization running from the menu bar. Use Quit from the menu-bar menu when you want to stop the app completely. Background discovery follows your registered Wi-Fi, interval and duration settings."
                    },
                )
            }
        }
