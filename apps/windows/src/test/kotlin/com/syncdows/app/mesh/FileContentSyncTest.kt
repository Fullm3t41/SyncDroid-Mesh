...
class FileContentSyncTest {
    ...
    fun wholeFilesUpdatesAndDeletionsSyncBetweenConfiguredPeers() = runBlocking {
        val directory = Files.createTempDirectory("syncdows-content-session")
        ...
        MeshStore(directory.resolve("first.db")).use { firstStore ->
            MeshStore(directory.resolve("second.db")).use { secondStore ->
                val profile = firstStore.createMesh("Content mesh", "First Mac", firstIdentity)
    ...
    fun partialBlocksResumeAfterStoreIsReopened() {
        val directory = Files.createTempDirectory("syncdows-resume")
    ...
    fun concurrentEditsCanKeepBothWithANumberedCopy() = runBlocking {
        val directory = Files.createTempDirectory("syncdows-conflict-session")
        ...
        MeshStore(directory.resolve("first.db")).use { firstStore ->
            MeshStore(directory.resolve("second.db")).use { secondStore ->
                val profile = firstStore.createMesh("Conflict mesh", "First PC", firstIdentity)
                val parents = firstStore.membershipEvents(profile.groupId)
                firstStore.applyMembership(
                    profile.groupName,
                    MembershipEvent.createAddDevice(
    ...
    private suspend fun syncOnce(
        ...
        firstIdentity: WindowsDeviceIdentity,
    ...
    private fun signedFolder(groupId: String, signer: WindowsDeviceIdentity): FolderAnnouncement {
    ...
    private fun memoryIdentity(alias: String): WindowsDeviceIdentity {
        val path: Path = Files.createTempDirectory("syncdows-content-identity").resolve("identity.p12")
