...
import com.syncdeck.app.mesh.CloudAccountStatus
...
fun BackgroundOperationScreen(
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        LazyColumn(
            ...
            item {
                Text(
                    "Keep SyncDeck available for scheduled synchronization without leaving its main window open.",
                    style = MaterialTheme.typography.bodyLarge,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
            }
            item {
                ...
                SettingsCard {
                    SettingsActionRow(
                        ...
                        detail = when {
                            !launchAtLogin -> "SyncDeck starts only when you open it"
                            noBackgroundService -> "Starts with its window open when you sign in"
                            else -> "Starts hidden in the menu bar"
                        },
                    ...
                    SettingsActionRow(
                        ...
                        detail = if (noBackgroundService) "Closing the window fully quits SyncDeck" else "Closing keeps SyncDeck in the menu bar",
            ...
            item {
                SettingsInformationCard(
                    if (noBackgroundService) {
                        "SyncDeck will stop discovery and synchronization when its window closes. Open the app again whenever you want to resume."
                    } else {
                        "Closing the SyncDeck window keeps synchronization running from the menu bar. Use Quit from the menu-bar menu when you want to stop the app completely. Background discovery follows your registered Wi-Fi, interval and duration settings."
                    },
                )
            }
        }
